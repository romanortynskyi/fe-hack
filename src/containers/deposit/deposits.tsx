...
const Deposits = () => {
    ...
    const [updateDeposit, { isLoading: isUpdateDepositLoading }] =
        useUpdateDepositMutation();
    const [addDeposit, { isLoading: isAddDepositLoading }] = useAddDepositMutation();
    ...
    const selectedDeposit= useMemo(
        () =>
            deposits?.find((deposit: depositEntity) => deposit.id === selectedDepositId),
        [selectedDepositd, deposits]
    );
    ...
    const onSubmit = async (values) => {
        try {
            if (selectedDepositd) {
                ...
                const updatedDeposit = deposits.map((deposit) =>
                    deposit.id === selectedDepositd ? updatedDeposit: deposit
                );
                setDeposits(updatedDeposit);
    ...
    const formik = useFormik({
        initialValues: {
            ...
            date: selectedDeposit?.date || new Date(),
        },
        onSubmit,
    });
