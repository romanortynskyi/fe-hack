...
const Incomes = () => {
  ...
  const { data: incomesFromQuery, isLoading: isIncomesLoading } =
    useGetIncomesQuery({
  ...
  const [updateIncome, { isLoading: isUpdateIncomeLoading }] =
    useUpdateIncomeMutation();
  const [addIncome, { isLoading: isAddIncomeLoading }] = useAddIncomeMutation();
  const [deleteIncome, { isLoading: isDeleteIncomeLoading }] =
    useDeleteIncomeMutation();
    ...
    try {
      if (selectedIncomeId) {
        ...
        const updatedIncomes = incomes.map((income) =>
          income.id === selectedIncomeId ? updatedIncome : income
        );
        setIncomes(updatedIncomes);
  ...
  return isIncomesLoading ? (
    ...
    <>
      ...
      />
      <Button onClick={onAddIncome} variant="contained">
        Add income
      </Button>
      <Modal
