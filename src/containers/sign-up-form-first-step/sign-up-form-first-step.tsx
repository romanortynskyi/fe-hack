...
const SignUpFormFirstStep = () => {
  ...
  const [checkIfUserExistsByEmail, { isLoading: isUserExistsByEmailLoading }] =
    userApi.endpoints.checkIfUserExistsByEmail.useLazyQuery();

  const { firstName, lastName, email, image } = signUpFormData;
  ...
  const initialValues =
    necessaryValues !== signUpFirstStepInitialValues
      ? necessaryValues
